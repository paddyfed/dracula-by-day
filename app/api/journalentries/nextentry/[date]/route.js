...
import { datePlusOneDay } from "@/app/utils/dateHelpers";
...
function getLatestJournalEntries(date, filtered) {
  ...
  const tomorrow = datePlusOneDay(date);

  return (filtered = getLatestJournalEntries(tomorrow, filtered));
}

export async function GET(request, props) {
  const params = await props.params;
  const date = datePlusOneDay(params.date);
